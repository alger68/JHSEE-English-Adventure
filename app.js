...
function home() {
  ...
  <div class="lesson-grid">${chapterLessons.map(l => {
    const complete = done.includes(l.day), isNext = l.day===next?.day;
    const best = state.completions[l.day]?.best;
    return `<article class="lesson-card jh-card ${complete?'completed':''} ${isNext?'current':''} ${bossLesson(l)?'boss-card':''} ${reviewLesson(l)?'week-finish':''}"><div class="lesson-card-head"><span class="day-square">${complete?'✓':String(l.day).padStart(2,'0')}</span>${!reviewLesson(l)?`<span class="chip">${bossLesson(l)?'BOSS':esc(l.type)}</span>`:''}</div><div class="${reviewLesson(l)?'finish-content':''}"><h3 lang="en">${esc(l.title)}</h3><p>${esc(l.goal)}</p></div><div class="card-foot"><span>${complete?(best===undefined?'V1 已完成':`最佳 ${best}%`):`約 ${l.minutes} 分鐘`}</span><a class="card-action" href="#lesson/${l.day}">${complete?'再練一次':'進入關卡'} →</a></div></article>`;
  }).join('')}</div>
  <div class="info-strip"><p>全部已發布關卡都已開放。完成 Boss 後，別忘了用錯題復活賽找回失分。</p><a class="text-link nowrap" href="#dashboard">查看學習紀錄 →</a></div><p class="difficulty-note">已收錄 ${lessons.length} 篇${lessonMeta.lastPublishDate ? ` · 最新發布 ${esc(lessonMeta.lastPublishDate)}` : ""}。250 天完整課程已收錄，可自由選關或依每日計畫練習。</p>`;
}
...
function lessonView(day,showLast=false,reset=false) {
  ...
  <div class="reading-layout"><section class="panel jh-card reading-panel"><div class="eyebrow">READ & DISCOVER</div><div class="story" lang="en">${storyHTML(l)}</div></section><aside class="reading-side"><section class="panel jh-card"><h3>今日核心單字</h3><div class="word-list">${l.words.map(w=>`<div class="word-row"><strong lang="en">${esc(w.word)}</strong><span>${esc(w.meaning)}</span></div>`).join('')}</div></section><section class="panel jh-card"><h3>好用片語</h3>${l.phrases.map(p=>`<div class="phrase"><b lang="en">${esc(p.phrase)}</b><p>${esc(p.meaning)}</p></div>`).join('')}</section><div class="tip">先看懂大意，再回文章找支持答案的那一句。交卷後才會揭曉答案與解析。</div></aside></div>
  <section class="panel jh-card quiz-panel"><div class="quiz-intro"><div><h2>${bossLesson(l)?'每週 Boss Challenge':'輪到你找線索了'}</h2><p class="muted small">${qs.length} 題 · 每題只有一個最佳答案</p></div><div class="settings-row"><label for="lessonTier">練習難度</label><select id="lessonTier" ${quizResult?'disabled':''}>${[1,2,3].map(t=>`<option value="${t}" ${active.tier===t?'selected':''}>${tiers[t]} · ${C.questions(l,t).length} 題</option>`).join('')}</select></div></div><p class="difficulty-note">各級使用同一篇文章；切換後會在文末增加題目。基礎：核心題＋字義偵探；進階：加一題；挑戰：再加一題綜合練習。這是本站練習分級，並非會考成績預測。</p>

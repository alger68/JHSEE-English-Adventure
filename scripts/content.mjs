...
export function validateContent(documents){
 const lessons=[],dailyDates=new Set(),publicationDates=new Set(),sources=new Map();
 for(const {filename,data} of documents){
  ...
  const daily=filename.startsWith('daily/'),batch=filename.startsWith('releases/');
  if(daily){
  ...
  if(batch&&!validDate(data.publishDate))fail(`${filename}: invalid batch publication date`);
  if(daily||batch)publicationDates.add(data.publishDate);
  for(const l of data.lessons){lessons.push(l);sources.set(l,{daily,batch,filename,date:data.publishDate});}
 }
 ...
 for(const [i,l] of lessons.entries()){
  const {daily,batch,filename,date}=sources.get(l),prefix=`${filename} Day ${l.day}`;
  ...
  for(const q of l.questions){
   ...
   if((daily||batch)&&!norm(l.story).includes(norm(q.evidence)))fail(`${prefix} ${q.id}: evidence must be copied exactly from this story`);
   if(q.reasons!==undefined&&(!Array.isArray(q.reasons)||q.reasons.length!==4||q.reasons.some(r=>!nonempty(r,1000))))fail(`${prefix} ${q.id}: expected four option explanations`);
  }
  if(daily){
 ...
 return {lessons,lastPublishDate:[...publicationDates].sort().at(-1)||null,lastDailyPublishDate:[...dailyDates].sort().at(-1)||null,questionCount:seenIds.size};
}
export function loadContent(root){
 ...
 // A release manifest explicitly publishes prebuilt content without inventing
 // historical daily files or changing the original lesson/question identities.
 const releaseDirectory=path.join(directory,'releases');
 const releases=fs.existsSync(releaseDirectory)?fs.readdirSync(releaseDirectory).filter(n=>n.endsWith('.json')).sort():[];
 const releasedFiles=new Set();
 for(const name of releases){
  const filename=`releases/${name}`,manifest=JSON.parse(fs.readFileSync(path.join(releaseDirectory,name),'utf8'));
  if(manifest.version!==1||!validDate(manifest.publishDate)||!nonempty(manifest.batchId,100)||!Array.isArray(manifest.files)||manifest.files.length===0||manifest.files.length>250)fail(`${filename}: invalid release manifest`);
  const batchLessons=[];
  for(const entry of manifest.files){
   if(!/^prebuilt\/day-\d{3}\.json$/.test(entry?.path)||!/^[a-f0-9]{64}$/.test(entry?.sha256))fail(`${filename}: invalid prebuilt path or checksum`);
   if(releasedFiles.has(entry.path))fail(`${filename}: duplicate release reference ${entry.path}`);
   releasedFiles.add(entry.path);
   const bytes=fs.readFileSync(path.join(directory,entry.path));
   if(crypto.createHash('sha256').update(bytes).digest('hex')!==entry.sha256)fail(`${filename}: checksum mismatch for ${entry.path}; published content must not silently change`);
   const source=JSON.parse(bytes);
   if(source.version!==2||source.status!=='prebuilt'||source.batchId!==manifest.batchId||source.lessons?.length!==1||entry.path!==`prebuilt/day-${String(source.lessons[0].day).padStart(3,'0')}.json`)fail(`${filename}: prebuilt source does not match manifest`);
   batchLessons.push(source.lessons[0]);
  }
  documents.push({filename,data:{version:2,publishDate:manifest.publishDate,lessons:batchLessons}});
 }

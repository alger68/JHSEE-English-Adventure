...
**2026-09-14 更新：使用者要求全部剩餘天數製作並發布。** Day 1–250 已完整收錄，共 250 篇、1,608 題。Day 13–35 與 Day 36–250 分別由 `content/releases/2026-09-14-day-013-035.json`、`content/releases/2026-09-14-day-036-250.json` 發布。所有已發布課程立即可選，不依個人日期鎖定。`prebuilt` 是來源檔存放位置，是否已發布以日更檔與 release manifest 為準。

**完成題庫後，日更改為引用現有教材與複習。** 不生成 Day 251、不建立同一課的重複日更檔，也不重建或更改既有排程。每日先找當日已發布檔或 `plannedPublishDate` 對應教材；原期程涵蓋 Day 13（2026-09-14）至 Day 250（2027-05-09）。這些是閱讀提醒日期，不是延後開放日期。期程結束後可從已發布課程挑選複習，明確稱「複習 Day N」。不得聲稱讀到了學生瀏覽器內的私人錯題或成績。

完整題本與下載：https://alger68.github.io/JHSEE-English-Adventure/downloads/ 。課程目錄與逐日目標見 [COURSE_250.md](COURSE_250.md)。
...
4. 當日檔案不存在且沒有當日已發布的提醒教材時，讀取種子週、日更與 release manifest 的 Day 編號；確認所有已發布 Day 連號後，以最大 Day + 1 新增。不能只看 `content/daily/` 而漏掉已批次發布的課程。再讀最近 7 份文章避免重複。不得重設為 Day 1，不得因漏跑而偽造過去日期。目前已達 Day 250，不再新增。先完成原定每日閱讀提醒；之後可循環選取既有課程複習，不把複習當作新增 Day。
...
若有本機執行環境，`node scripts/prebuilt.mjs --release` 依台灣日期輸出一個動作：`reuse-published` 表示引用已上線教材，沒有待建立檔；`review-only` 表示已滿 250 天，改為複習既有課程，沒有待新增檔；`create-daily` 僅適用尚未完成課程的歷史流程。命令只輸出，**不寫檔、不提交、不發布**。實際新增仍依第 7 步與部署查核辦理。預備題库只有在 release manifest 明確列入時才由網站建置收錄，詳見 [PREBUILT_BANK.md](PREBUILT_BANK.md)。GitHub Actions 本身只在推送或手動執行時啟動；本次不建立或更改排程時間。
